...
static mrb_value drb_ffi_hello_Binding(mrb_state *state, mrb_value value) {
    hello();
...
void drb_register_c_extensions(void *(*lookup)(const char *), mrb_state *state, struct RClass *FFI) {
    ...
    mrb_define_module_function_f(state, module, "hello", drb_ffi_hello_Binding, MRB_ARGS_REQ(0));
}
